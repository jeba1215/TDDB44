...
quad_list::quad_list(int ll) :
		head(NULL), tail(NULL), last_label(ll) {
	quad_nr = 1;
}
...
sym_index ast_expr_list::generate_quads(quad_list &q) {
	...
	fatal(
			"Trying to call generate_quads for ast_expr_list. Try 'generate_parameter_list' instead.");
	return NULL_SYM;
}
...
sym_index ast_integer::generate_quads(quad_list &q) {
	...
	;
	/* Your code here */
	sym_index i = sym_tab->gen_temp_var(integer_type);
	q += new quadruple(q_iload, this->value, NULL_SYM, i);
	return i;
}
...
sym_index ast_real::generate_quads(quad_list &q) {
	...
	;
	/* Your code here */
	sym_index i = sym_tab->gen_temp_var(real_type);
	q += new quadruple(q_rload, sym_tab->ieee(this->value), NULL_SYM, i);
	return i;
}
...
sym_index ast_not::generate_quads(quad_list &q) {
	...
	;
	/* Your code here */
	sym_index i = this->expr->generate_quads(q);
	sym_index temp = sym_tab->gen_temp_var(integer_type);
	q += new quadruple(q_inot, i, NULL_SYM, temp);
	return temp;
}
...
sym_index ast_uminus::generate_quads(quad_list &q) {
	...
	if (sym_tab->get_symbol(i)->type == integer_type) {
		sym_index temp = sym_tab->gen_temp_var(integer_type);
		q += new quadruple(q_iuminus, i, NULL_SYM, temp);
		return temp;
	} else if (sym_tab->get_symbol(i)->type == real_type) {
		sym_index temp = sym_tab->gen_temp_var(real_type);
		q += new quadruple(q_ruminus, i, NULL_SYM, temp);
		return temp;
	}
...
sym_index ast_cast::generate_quads(quad_list &q) {
	...
	if (sym_tab->get_symbol(i)->type == integer_type) {
		sym_index temp = sym_tab->gen_temp_var(real_type);
		q += new quadruple(q_itor, i, NULL_SYM, temp);
		return temp;
	}

	fatal("trying to cast something other than int");
	return NULL_SYM;
}

sym_index generate_quad_binop(quad_op_type q_, ast_binaryoperation *node,
		quad_list &q) {
	sym_index ileft = node->left->generate_quads(q);
	sym_index iright = node->right->generate_quads(q);
	sym_index temp = sym_tab->gen_temp_var(node->type);
	q += new quadruple(q_, ileft, iright, temp);
	return temp;
}
...
sym_index ast_add::generate_quads(quad_list &q) {
	...
	if (this->type == integer_type)
		return generate_quad_binop(q_iplus, this, q);
	else if (this->type == real_type)
		return generate_quad_binop(q_rplus, this, q);
	else
		fatal("strange type in generate quad");
	return NULL_SYM;
}
...
sym_index ast_sub::generate_quads(quad_list &q) {
	...
	/* Your code here */
	if (this->type == integer_type)
		return generate_quad_binop(q_iminus, this, q);
	else if (this->type == real_type)
		return generate_quad_binop(q_rminus, this, q);
	else
		fatal("strange type in generate quad");
	return NULL_SYM;
}
...
sym_index ast_mult::generate_quads(quad_list &q) {
	...
	/* Your code here */
	if (this->type == integer_type)
		return generate_quad_binop(q_imult, this, q);
	else if (this->type == real_type)
		return generate_quad_binop(q_rmult, this, q);
	else
		fatal("strange type in generate quad");
	return NULL_SYM;
}
...
sym_index ast_divide::generate_quads(quad_list &q) {
	...
	;
	/* Your code here */
	return generate_quad_binop(q_rdivide, this, q);
}
...
sym_index ast_idiv::generate_quads(quad_list &q) {
	...
	;
	/* Your code here */
	return generate_quad_binop(q_idivide, this, q);
}
...
sym_index ast_mod::generate_quads(quad_list &q) {
	...
	;
	/* Your code here */
	return generate_quad_binop(q_imod, this, q);
}
...
sym_index ast_or::generate_quads(quad_list &q) {
	...
	;
	/* Your code here */
	return generate_quad_binop(q_ior, this, q);
}
...
sym_index ast_and::generate_quads(quad_list &q) {
	...
	;
	/* Your code here */
	return generate_quad_binop(q_iand, this, q);
}
...
sym_index ast_equal::generate_quads(quad_list &q) {
	...
	;
	/* Your code here */
	sym_index ileft = this->left->generate_quads(q);
	sym_index iright = this->right->generate_quads(q);
	sym_index temp = sym_tab->gen_temp_var(this->type);
	if (this->type == integer_type)
		q += new quadruple(q_ieq, ileft, iright, temp);
	else
		q += new quadruple(q_req, ileft, iright, temp);
...
sym_index ast_notequal::generate_quads(quad_list &q) {
	...
	if (this->left->type == integer_type)
		q += new quadruple(q_ine, ileft, iright, temp);
	else
		q += new quadruple(q_rne, ileft, iright, temp);
...
sym_index ast_lessthan::generate_quads(quad_list &q) {
	...
	sym_index temp = sym_tab->gen_temp_var(this->type);
	if (this->left->type == integer_type)
		q += new quadruple(q_ilt, ileft, iright, temp);
	else
		q += new quadruple(q_rlt, ileft, iright, temp);
...
sym_index ast_greaterthan::generate_quads(quad_list &q) {
	...
	sym_index temp = sym_tab->gen_temp_var(this->type);
	if (this->left->type == integer_type)
		q += new quadruple(q_igt, ileft, iright, temp);
	else
		q += new quadruple(q_rgt, ileft, iright, temp);
...
void ast_elsif_list::generate_quads_and_jump(quad_list &q, int label) {
	...
	if (this->preceding != NULL) {
		this->preceding->generate_quads_and_jump(q, label);
	}
	if (this->last_elsif != NULL) {
		this->last_elsif->generate_quads_and_jump(q, label);
	}
}
...
void quadruple::print(ostream &o) {
	...
	case q_itor:
		...
		break;
	case q_jmp:
		o << setw(11) << "q_jmp" << setw(11) << int1 << setw(11) << "-"
				<< setw(11) << "-";
		break;
	case q_jmpf:
	...
	case q_param:
		...
		break;
	case q_labl:
		o << setw(11) << "q_labl" << setw(11) << int1 << setw(11) << "-"
				<< setw(11) << "-";
		break;
	case q_nop:
		o << setw(11) << "q_nop" << setw(11) << "-" << setw(11) << "-"
				<< setw(11) << "-";
		break;
	default:
